...
const adminController = {
  ...
  async getAllAccounts(req, res) {
    try {
      ...
      const adminData = admins.map((admin) => ({
        ...admin.dataValues,
        role: "Admin",
      })); // Extract dataValues for admins and add role
      const userData = users.map((user) => ({
        ...user.dataValues,
        role: "User",
      })); // Extract dataValues for users and add role
  ...
  async getPaginatedAccounts(offset, limit) {
    try {
      ...
      const adminData = admins.map((admin) => ({
        ...admin.dataValues,
        role: "Admin",
        createdAt: moment(admin.dataValues.createdAt).format(
      ...
      const userData = users.map((user) => ({
        ...user.dataValues,
        role: "User",
        createdAt: moment(user.dataValues.createdAt).format(
          "DD/MM/YYYY, h:mm:ss a"
        ),
      })); // Extract dataValues for users and add role
  ...
  async changePassword(req, res) {
    try {
      ...
      if (
        !(await adminService.validatePassword(
          oldPassword,
